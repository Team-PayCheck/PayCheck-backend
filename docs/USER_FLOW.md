...
- **근무 기록 (CONFIRMED)**: 실제 근무가 완료되어 확정된 기록
...
INSERT INTO WorkRecord
  (contract_id, work_date, scheduled_start_time, scheduled_end_time,
   status, memo)
VALUES
...
**중요**:
- `actual_start_time`, `actual_end_time`은 **NULL** (아직 근무 안 함)
- `total_hours`, `regular_hours` 등도 **NULL** 또는 **0**
- 상태는 **SCHEDULED** (근무 예정)
...
**데이터베이스 변화:**
```sql
-- WorkRecord 업데이트 (아직 SCHEDULED 상태)
UPDATE WorkRecord
...
11월 1일 퇴근 후:
[박성호 사장] 김민지 근무 확정
  → WorkRecord #1 업데이트 (status: CONFIRMED)
  → actual_start_time, actual_end_time 입력
