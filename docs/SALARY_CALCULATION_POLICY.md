...
#### 소득세 계산 (간이세액표 기준)
- **간이세액표(조견표) 조회**로 소득세를 계산한다. (월급여액: 비과세/학자금 제외)
- **부양가족 수 입력 필드가 없으므로 기본값 1명으로 고정**하여 조회한다.
- **10,000천원 초과 구간은 표 하단 수식 규칙**을 적용한다.
- **지방소득세**: 소득세의 10%
- **8~20세 자녀 추가공제**는 입력 필드가 없어 **현재 미적용** (추후 입력 추가 시 반영)
